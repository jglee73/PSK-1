...
int CMacro_Log::Get_ModuleType(const CString& module)
{
	...
	// LLx ...
	for(i=0; i<SCH_DB__LLx_LIMIT; i++)
	{
		CString ll_name = "LBx";

			 if(i == 0)		ll_name = "LBA";
		else if(i == 1)		ll_name = "LBB";
		else if(i == 2)		ll_name = "LBC";
		else if(i == 3)		ll_name = "LBD";
		else				continue;

		for(int k=0; k<SCH_DB__LLx_SLOT_LIMIT; k++)
		{
			str_module.Format("%s_%1d",  ll_name,k+1);
...
int CMacro_Log::Lot_Write(const CString& lotid,const CString& portid)
{
	/*
