		...
		if(timer_count % 5 == 0)
		{
			...
			if((cur_id > 0) && (cur_id <= iPMC_SIZE))
			{
				...
				// PART ITEM ...
				{
					...
					{
						sEXT_CH__PMC_PART_HTRx_TEMP_SET[db_index][i]->Get__DATA(str_data);
						sCH__PMC_PART_HTRx_TEMP_SET[i]->Set__DATA(str_data);

						sEXT_CH__PMC_PART_HTRx_TEMP_GET[db_index][i]->Get__DATA(str_data);
						sCH__PMC_PART_HTRx_TEMP_GET[i]->Set__DATA(str_data);
					}
			...
			else
			{
				...
				// PART ITEM ...
				{
					...
					for(i=0; i<iHTR_SIZE; i++)
					{
						sCH__PMC_PART_HTRx_TEMP_SET[i]->Set__DATA("");
						sCH__PMC_PART_HTRx_TEMP_GET[i]->Set__DATA("");
					}
