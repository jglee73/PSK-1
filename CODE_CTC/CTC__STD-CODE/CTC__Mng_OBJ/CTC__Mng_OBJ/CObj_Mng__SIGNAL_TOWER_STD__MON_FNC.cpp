...
//-------------------------------------------------------------------------
int  CObj_Mng__SIGNAL_TOWER_STD
	...
	while(1)
	{
		...
		// YELLOW ---------------
		{
			...
			switch(flag)
			{
				...
				default:
					Fnc__Set_Off_Light_Color(STR__YELLOW_LIGHT);
					break;
			}
		}
		...
		// GREEN ---------------
		{
			...
			switch(flag)
			{
				...
				default:
					Fnc__Set_Off_Light_Color(STR__GREEN_LIGHT);
					break;
			}
		}
...
//-------------------------------------------------------------------------
// Alarm & Maint
int  CObj_Mng__SIGNAL_TOWER_STD::Check__ALARM_Or_MAINT()
{
	int error_flag = -1;
	int maint_flag = -1;
...
// Cassette Status
int  CObj_Mng__SIGNAL_TOWER_STD::Check__LOAD_REQ_Or_UNLOAD_REQ()
{
...
// Process & Idle 
int  CObj_Mng__SIGNAL_TOWER_STD::Check__PROCESS_Or_IDLE()
{
	if(xSCH_MATERIAL_CTRL->Check__SYSTEM_STATUS__PROCESS() > 0)
	{
		iPROCESS_PRE_FLAG = 1;
...
// Control State
int  CObj_Mng__SIGNAL_TOWER_STD::Check__FA_STATUS()
{
	int fa_sts = xSCH_MATERIAL_CTRL->Get__FA_STATUS();

	switch(fa_sts)
	{
		case FA_STATUS__REMOTE:			
			{
				CString main_name;
				CString sub_name;
