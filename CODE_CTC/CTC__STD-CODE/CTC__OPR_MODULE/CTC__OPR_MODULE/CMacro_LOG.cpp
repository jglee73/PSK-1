...
int CMacro_LOG::Init_LOG()
{
	...
	xI_SYS_INFO->Get__DIR_ROOT(dir_root);
	//

	//.....
	xI_LOT_SUMMARY->Create__SERVICE_DB("");
	xI_LOT_SUMMARY->Link__SERVICE_DB("");

	xI_MATERIAL_LOG->Create__SERVICE_DB("");
	xI_MATERIAL_LOG->Link__SERVICE_DB("");
	//

	//.....
	xI_LOT_SUMMARY->Set__MULTI_SLOT();
	xI_LOT_SUMMARY->Set__DIR_ROOT(dir_root); 
	xI_LOT_SUMMARY->Set__DATE_LIMIT(90);

	xI_MATERIAL_LOG->Set__DIR_ROOT(dir_root);
	xI_MATERIAL_LOG->Set__DATE_LIMIT(90);
	//
...
int CMacro_LOG::Get_ModuleType(const CString& module)
{
	...
	for(i=0; i<CFG_LLx__SLOT_MAX; i++)
	{
		str_module.Format("SLA_%1d",i+1);
		if(str_module.CompareNoCase(module) == 0)		return (71+i);
	}
	for(i=0; i<CFG_LLx__SLOT_MAX; i++)
	{
		str_module.Format("SLB_%1d",i+1);
...
int CMacro_LOG::Log_Material_Picking(const CString& arm,const CString& str_module,
...
int CMacro_LOG::Log_Material_Pick(const CString& arm,const CString& str_module,
								  const CString& material,const CString& lotid,const CString& recipe,
								  int  ex_flag)
{
	int arm_type = Get_ArmType(arm);
...
int CMacro_LOG::Log_Material_Placing(const CString& arm,const CString& str_module,
									 const CString& material,const CString& lotid,const CString& recipe,
									 int  ex_flag)
{
	int arm_type = Get_ArmType(arm);
...
int CMacro_LOG::Log_Material_Place(const CString& arm,const CString& str_module,
								   const CString& material,const CString& lotid,const CString& recipe,
								   int  ex_flag)
{
	int arm_type = Get_ArmType(arm);
...
int CMacro_LOG::Lot_Write_Ex(const CString& lotid,
							 const CString& portid,
							 const CString& user_id,
							 const CString& user_level)
{
	xI_LOT_SUMMARY->Clear__LOT_LOG(lotid,portid);
	xI_LOT_SUMMARY->Write__LOT_LOG_EX(lotid,portid,user_id,user_level);

	xI_MATERIAL_LOG->Start__LOG(lotid,portid);
	return 1;
}

int CMacro_LOG::Lot_End_Write(const CString& lotid,const CString& portid)
{
	xI_LOT_SUMMARY->End__LOT_LOG(lotid,portid);

	xI_MATERIAL_LOG->End__LOG(lotid,portid);
	return 1;
}

int CMacro_LOG::Lot_Slot_Write(const CString& lotid,const CString& portid,int slot,
...
int CMacro_LOG::Lot_Slot_End_Write(const CString& lotid,const CString& portid,int slot)
{
	xI_LOT_SUMMARY->End__SLOT_LOG(lotid,portid,slot);
	return 1;
}

int CMacro_LOG::Lot_Slot_AlarmPost(const CString& lotid,const CString& portid,int slot,int alarmid)
{
	CString prcid = "AAA";
...
int CMacro_LOG::Lot_Slot_AlarmClear(const CString& lotid,const CString& portid,int slot,int alarmid)
{
