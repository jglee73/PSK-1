...
CDRV__ADS_CTRL mDRV__ADS_CTRL; 


// ...
CDRV__ADS_CTRL::CDRV__ADS_CTRL()
{
...
// ...
int CDRV__ADS_CTRL::Init__ADS_DRIVER(const int active_sim)
{
	// DB.ADS_IN ...
	{
		...
		Init__ADS_INFO(p_ads_drv,p_ads_db, active_sim);
	}
	// DB.ADS_OUT ...
	{
...
CString CDRV__ADS_CTRL::Get__ADS_IN_INFO(const bool active_print)
{
	CString log_msg;
	CString log_bff;
	...
	{ 
		...
		mDB__ADS_CTRL.Load__ADS_IN_INFO(str_name, i_size, str_type);
...
int CDRV__ADS_CTRL::Link__ADS_OUT_INFO(const bool active_print)
{
	mDB__ADS_CTRL.Clear__ADS_OUT_ALL();

	// ...
	CStringArray l__ads_name;
	mCH__ADS_CTRL.Get__ADS_NAME_LIST(l__ads_name);

	int i_limit = l__ads_name.GetSize();
	int i;

	for(i=0; i<i_limit; i++)
	{	
		CString ads_name = l__ads_name[i];
		
		int data_size;
		CString data_type;

		if(mDB__ADS_CTRL.Get__ADS_IN_INFO(ads_name, data_size,data_type) < 0)		continue;

		int ads_index = mDB__ADS_CTRL.Load__ADS_OUT_INFO(ads_name, data_size,data_type);
		mCH__ADS_CTRL.Link__ADS_INDEX(i, ads_index);
	}

	return 1;
}

int CDRV__ADS_CTRL::Init__ADS_INFO(CDRV__ADS_INFO* p_ads_drv,CDB__ADS_INFO* p_ads_db, const int active_sim)
{
...
int CDRV__ADS_CTRL::Update__ADS_READING(const bool active_print, const int active_sim)
{
	if(active_sim > 0)
	{
...
int CDRV__ADS_CTRL::Update__ADS_SETTING(const bool active_print, const int active_sim)
{
	if(active_sim > 0)
	{
	...
	// ...
	{
		...
		for(long idx=0; idx<item_count; idx++)
		{
			...
			{
				BYTE* p_addr_byte = pObjAdsRes + k;

				str_hexa = l_para[k];
				*p_addr_byte = 0x0ff & Macro__Hexa_From_String(str_hexa);
			}

			//
			pObjAdsRes = pObjAdsRes + data_size;
			pObjAdsErrRes = pObjAdsErrRes + 4;					// point to next ADS-err object
		}
	}
