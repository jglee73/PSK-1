...
::_Drv__CMMD_SEND(const CString& cmmd_org, CString* recv_data)
{
	...
	cmmd_set.Format("%s%c", cmmd_org, CR);
	cmmd_end.Format("%c%c", CR, LF);
...
LOOP_RETRY:
	...
	while(1)
	{
		...
		int r_flag = mX_Serial->DATA__RECV(cmmd_end, &r__data_1, 150);
		...
		if(recv_cnt == 1)
		{
			...
			if(r_bff.GetLength() > 0)
			{
				...
				if(s_index >= 0)
				{
					...
					//
					if(r__data_1.CompareNoCase(cmmd_org) != 0)		return -12;

					recv_data->Format("%s", r_bff);
					return 12;
				}
			}
		}
