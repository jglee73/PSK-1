...
int CObj__LV32_BLDC::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// CFG ...
	{
		...
		var_name = "CUR.FFU.TYPE";
		STD__ADD_STRING(var_name);
		LINK__VAR_STRING_CTRL(sCH__CUR_FFU_TYPE, var_name);
	...
	// BLOCK ...
	{
		for(i=0; i<CFG__FAN_SIZE; i++)
		{
			...
			//
			var_name.Format("FFU.BLOCK.ERROR.ACTIVE.%1d", id);
			STD__ADD_STRING(var_name);
			LINK__VAR_STRING_CTRL(sCH__FFU_BLOCK__ERROR_ACTIVE_X[i], var_name);
		}
	}
