...
// ...
void CObj__PROC_FNC
::Mon__SYSTEM_LOCK_CTRL(const int event_id, CII_OBJECT__VARIABLE *p_variable,CII_OBJECT__ALARM *p_alarm)
{
	HANDLE h_event = p_variable->Link__EVENT_HANDLE(event_id, 0.1);

	// ...
	CString cur__sys_status;
	CString var_data;


	while(1)
	{
		p_variable->Wait__EVENT_HANDLE(h_event);


		// ...
		{
			dEXT_CH__SYS_STATUS->Get__DATA(cur__sys_status);
		}
		...
		// SYSTEM CTRL ...
		{
			...
			// ...
			sEXT_CH__SYS_CTRL_LOCK->Get__DATA(var_data);
...
void CObj__PROC_FNC::
Mon__PROC_EXCEPTION_CTRL(const int event_id, CII_OBJECT__VARIABLE *p_variable,CII_OBJECT__ALARM *p_alarm)
{
	HANDLE h_event = p_variable->Link__EVENT_HANDLE(event_id, 0.1);
