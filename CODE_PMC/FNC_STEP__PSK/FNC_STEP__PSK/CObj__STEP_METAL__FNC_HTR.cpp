...
::_Fnc__HTR_STABLE_CONTROL(CII_OBJECT__VARIABLE *p_variable,CII_OBJECT__ALARM* p_alarm)
{
	...
	if(bActive__OBJ_CTRL__HTR_CHUCK)
	{
		...
		{
			CString rcp__temp    = sCH__RCP_ESC_CHUCK_TEMP_X[i]->Get__STRING();
			CString rcp__offset  = sCH__RCP_ESC_CHUCK_OFFSET_X[i]->Get__STRING();
			CString rcp__warning = sCH__RCP_ESC_CHUCK_WARNING_X[i]->Get__STRING();
			CString rcp__alarm   = sCH__RCP_ESC_CHUCK_ALARM_X[i]->Get__STRING();
