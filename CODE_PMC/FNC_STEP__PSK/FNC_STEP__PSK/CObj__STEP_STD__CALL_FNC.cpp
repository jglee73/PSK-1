...
::Call__PROC(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	for(i=0; i<iDATA__MFC_SIZE; i++)
	{
		MFC_OBJ__Start_CONTROL(i, rcp__mfc_x_flow[i]);
	}
	...
	while(1)
	{
		...
		// ERROR CHECK ...
		{
			if(MFC_OBJ__Check_ERROR() > 0)			return -11;
			if(APC_OBJ__Check_ERROR() > 0)			return -12;
			if(PT_OBJ__Check_ERROR()  > 0)			return -13;

			if(RF_SRC_OBJ__Check_ERROR()  > 0)		return -21;
			if(RF_BIAS_OBJ__Check_ERROR() > 0)		return -22;
		}
