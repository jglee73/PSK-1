...
public:
	...
	void Reset()
	{
		...
		if(!bActive__REF_OUTPUT)		dPre__Set_Value = -1.0;
	}
...
private:
	...
	void _Check__Error_State()
	{
		...
		if(bActive__REF_OUTPUT)
		{
			stable_min = set_value - err_stable;
			stable_max = set_value + err_stable;

			err_min = set_value - err_value;
			err_max = set_value + err_value;
		...
		else
		{
			...
			if(pCH__CUR_STABLE_MIN != NULL)
			{
				ch_data = pCH__CUR_STABLE_MIN->Get__STRING();
				stable_min = atof(ch_data);
			}
			if(pCH__CUR_STABLE_MAX != NULL)
			{
				ch_data = pCH__CUR_STABLE_MAX->Get__STRING();
				stable_max = atof(ch_data);
			}

			if(pCH__CUR_ERROR_MIN != NULL)
			{
				ch_data = pCH__CUR_ERROR_MIN->Get__STRING();
				err_min = atof(ch_data);
			}
			if(pCH__CUR_ERROR_MAX != NULL)
			{
				ch_data = pCH__CUR_ERROR_MAX->Get__STRING();
				err_max = atof(ch_data);
			}			
		}
		...
		if(((err_min > 0.1) && (cur_value < err_min))
		|| ((err_max > 0.1) && (cur_value > err_max)))
		{
			bActive__Stable_State = false;
