...
::Mon__ERROR_CHECK(CII_OBJECT__VARIABLE *p_variable, CII_OBJECT__ALARM *p_alarm)
{
	...
	for(i=0; i<iLOOP_SIZE; i++)
	{
		...
		// ...
		{
			...
			x_link_common.pCH__ACTIVE_CHECK = dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i].Get__PTR();

			x_link_common.pCH__CFG_STABLE_CHECK_SEC = aCH__CFG_STABLE_CHECK_TIMEOUT__LOOP_X[i].Get__PTR();
			x_link_common.pCH__CUR_STABLE_CHECK_SEC = sCH__MON_STABLE_CHECK_TIME__LOOP_X[i].Get__PTR();

			x_link_common.pCH__CFG_STABLE_DELAY_SEC = aCH__CFG_DELAY_CHECK_TIMEOUT__LOOP_X[i].Get__PTR();
			x_link_common.pCH__CUR_STABLE_DELAY_SEC = sCH__MON_DELAY_CHECK_TIME__LOOP_X[i].Get__PTR();

			x_link_common.pCH__CUR_ERROR_MIN = sCH__MON_WARNING_MIN__LOOP_X[i].Get__PTR();
			x_link_common.pCH__CUR_ERROR_MAX = sCH__MON_WARNING_MAX__LOOP_X[i].Get__PTR();
		}
		...
		error__pt_warning_x[i].Link__REF_OUT_CHANNEL(aCH__MON_IO_SET_SP__LOOP_X[i]);
		...
		x_link_para.pCH__CFG_TOLERANCE_DATA_PERCENT = aCH__CFG_SOFT_TOLERANCE_THRESHOLD__LOOP_X[i].Get__PTR();

		x_link_para.pCH__CFG_ERROR_CHECK_SEC = aCH__CFG_SOFT_TOLERANCE_TIMEOUT__LOOP_X[i].Get__PTR();
		x_link_para.pCH__CUR_ERROR_CHECK_SEC = sCH__MON_WARNING_CHECK_TIME__LOOP_X[i].Get__PTR();
	...
	for(i=0; i<iLOOP_SIZE; i++)
	{
		...
		// ...
		{
			...
			x_link_common.pCH__CUR_ERROR_MIN = sCH__MON_ABORT_MIN__LOOP_X[i].Get__PTR();
			x_link_common.pCH__CUR_ERROR_MAX = sCH__MON_ABORT_MAX__LOOP_X[i].Get__PTR();
		}
		...
		error__pt_abort_x[i].Link__REF_OUT_CHANNEL(aCH__MON_IO_SET_SP__LOOP_X[i]);
		...
		x_link_para.pCH__CFG_TOLERANCE_DATA_PERCENT = aCH__CFG_HARD_TOLERANCE_THRESHOLD__LOOP_X[i].Get__PTR();

		x_link_para.pCH__CFG_ERROR_CHECK_SEC = aCH__CFG_HARD_TOLERANCE_TIMEOUT__LOOP_X[i].Get__PTR();
		x_link_para.pCH__CUR_ERROR_CHECK_SEC = sCH__MON_ABORT_CHECK_TIME__LOOP_X[i].Get__PTR();
	...
	for(i=0; i<iLOOP_SIZE; i++)
	{
		dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Set__DATA(STR__READY);
	}
	...
	while(1)
	{
		...
		for(i=0; i<iLOOP_SIZE; i++)
		{
			if(dCH__CFG_USE__LOOP_X[i]->Check__DATA(STR__YES) < 0)
			{
				...
				dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Set__DATA(STR__OFF);
				dCH__MON_STABLE_ACTIVE__LOOP_X[i]->Set__DATA(STR__OFF);	
				dCH__MON_ABORT_ACTIVE__LOOP_X[i]->Set__DATA(STR__OFF);	
				continue;
			}
			else
			{
				if(dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Check__DATA(STR__OFF) > 0)
				{
					dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Set__DATA(STR__READY);
				}
			}

			if(dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Check__DATA(STR__READY) > 0)
			{
				dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Set__DATA(STR__ON);
			...
			// ...
			int active__error_check = dCH__MON_ERROR_CHECK_ACTIVE__LOOP_X[i]->Check__DATA(STR__ON);
			...
			// Abort Check ...
			{
				...
				if(p_error->Check__Error_State())
				{
					if(active__error_check > 0)
					{
						dCH__MON_ABORT_ACTIVE__LOOP_X[i]->Set__DATA(STR__ON);
			...
			if(active__error_check > 0)
			{
				...
				{
					dCH__MON_STABLE_ACTIVE__LOOP_X[i]->Set__DATA(STR__ON);	
					dCH__MON_ABORT_ACTIVE__LOOP_X[i]->Set__DATA(STR__OFF);
				}
				else
				{
