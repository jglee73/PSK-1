	...
	while(1)
	{
		...
		if(iActive__SIM_MODE > 0)
		{
			...
			if(bActive__ESC_POWER_CENTER_USE)
			{
				...
				var_data.Format("%.1f", sim__curr_value);
		...
		if(bActive__HE_CONTROL_CENTER_USE)
		{
			int he_flow_check = doEXT_CH__HE_VALVE_FINAL_OUT_CENTER->Check__DATA(STR__Open);
		...
		if(bActive__HE_CONTROL_EDGE_USE)
		{
			int he_flow_check = doEXT_CH__HE_VALVE_FINAL_OUT_EDGE->Check__DATA(STR__Open);
...
Mon__ESC_STABLE_CHECK(CII_OBJECT__VARIABLE *p_variable, CII_OBJECT__ALARM *p_alarm)
{
	...
	while(1)
	{
		...
		//  STABLE.CHECK ...
		{
			...
			if(active__stable_check)			dCH__MON_STABLE_ACTIVE->Set__DATA(STR__ON);
			else								dCH__MON_STABLE_ACTIVE->Set__DATA(STR__OFF);
		}
		...
		// FAULT.CHECK ...
		{
			...
			if(bActive__ESC_POWER_CENTER_USE)
			{
				if(sCH__MON_CENTER_FAULT_ESC_STATE->Check__DATA(STR__YES) > 0)			active__fault_check = true;
			}
			if(bActive__HE_CONTROL_CENTER_USE)
			{
			...
			if(bActive__ESC_POWER_EDGE_USE)
			{
				if(sCH__MON_EDGE_FAULT_ESC_STATE->Check__DATA(STR__YES) > 0)			active__fault_check = true;
			}
			if(bActive__HE_CONTROL_EDGE_USE)
			{
			...
			else								dCH__MON_FAULT_ACTIVE->Set__DATA(STR__OFF);
		}
