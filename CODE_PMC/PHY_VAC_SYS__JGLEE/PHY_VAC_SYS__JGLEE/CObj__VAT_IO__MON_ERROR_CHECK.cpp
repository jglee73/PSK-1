...
::Mon__ERROR_CHECK(CII_OBJECT__VARIABLE *p_variable, CII_OBJECT__ALARM *p_alarm)
{
	...
	// Range.Warning ...
	{
		...
		// ...
		{
			...
			x_link_common.pCH__ACTIVE_CHECK = dCH__MON_PRESSURE_CHECK_ACTIVE.Get__PTR();

			x_link_common.pCH__CFG_STABLE_DELAY_SEC = aCH__CFG_PRESSURE_STABLE_DELAY_TIME.Get__PTR();
			x_link_common.pCH__CUR_STABLE_DELAY_SEC = sCH__MON_PRESSURE_STABLE_DELAY_TIME.Get__PTR();

			x_link_common.pCH__CFG_STABLE_CHECK_SEC = aCH__CFG_PRESSURE_STABLE_CHECK_TIME.Get__PTR();
			x_link_common.pCH__CUR_STABLE_CHECK_SEC = sCH__MON_PRESSURE_STABLE_CHECK_TIME.Get__PTR();

			x_link_common.pCH__CUR_ERROR_MIN = sCH__MON_PRESSURE_STABLE_MIN.Get__PTR();
			x_link_common.pCH__CUR_ERROR_MAX = sCH__MON_PRESSURE_STABLE_MAX.Get__PTR();
		...
		// Process ...
		{
			...
			x_link_para.pCH__CUR_ERROR_CHECK_SEC        = sCH__MON_PRESSURE_WARNING_CHECK_TIME.Get__PTR();
	...
	// Range.Abort ...
	{
		...
		// ...
		{
			...
			x_link_common.pCH__CUR_ERROR_MIN = sCH__MON_PRESSURE_ABORT_MIN.Get__PTR();
			x_link_common.pCH__CUR_ERROR_MAX = sCH__MON_PRESSURE_ABORT_MAX.Get__PTR();
		}
		...
		// Process ...
		{
			...
			x_link_para.pCH__CUR_ERROR_CHECK_SEC        = sCH__MON_PRESSURE_ABORT_CHECK_TIME.Get__PTR();
	...
	while(1)
	{
		...
		if(dCH__MON_PRESSURE_CHECK_ACTIVE->Check__DATA(STR__READY) > 0)
		{
			dCH__MON_PRESSURE_CHECK_ACTIVE->Set__DATA(STR__ON);
		...
		// ...
		int active__error_check = dCH__MON_PRESSURE_CHECK_ACTIVE->Check__DATA(STR__ON);
		...
		if(active__error_check > 0)
		{
			if(active__proc_state > 0)   // Process State ...
			{
				...
				if(p_error->Check__Error_State())
				{
					if(active__error_check > 0)
					{
						dCH__MON_PRESSURE_ABORT_ACTIVE->Set__DATA(STR__ON);
