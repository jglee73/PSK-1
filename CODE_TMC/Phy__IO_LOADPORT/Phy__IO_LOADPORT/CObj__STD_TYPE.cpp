...
int CObj__STD_TYPE::__DEFINE__VARIABLE_STD(p_variable)
{
	...
	// ...
	{
		p_variable->Add__MONITORING_PROC(7.0,MON_ID__ALL_STATE);
		p_variable->Add__MONITORING_PROC(1.0,MON_ID__BUTTON_PROC);
	}
	return 1;
}
int CObj__STD_TYPE::__DEFINE__ALARM(p_alarm)
{
	...
	// ...
	{
		alarm_id = ALID__LP_WITH_NO_RF_TAG_SENSOR;

		alarm_title  = title;
		alarm_title += "LP has no RF-tag reader !";
...
int CObj__STD_TYPE::__CALL__MONITORING(id, p_variable, p_alarm)
{
	switch(id)
	{
		...
		case MON_ID__BUTTON_PROC:	
			Mon__Button_Proc(p_variable,p_alarm);
			break;
	}
