...
::Call__INIT(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	if(pTMC_CHM__OBJ_CTRL->Call__OBJECT(CMMD__INIT) < 0)
	{
		return -1;
	}
...
{
	return pTMC_CHM__OBJ_CTRL->Call__OBJECT("CL.ALL_VLV");
}
...
Mode_Chk_Rty:
	...
	{
		dEXT_CH__LLx__PUMPING_STS_FLAG_X[i]->Link__UPPER_OBJECT_ABORT(sObject_Name);
		dEXT_CH__LLx__PUMPING_STS_FLAG_X[i]->Enable__UPPER_OBJECT_ABORT_CHECK();
	}
	...
	for(i=0; i<iDATA__LLx_SIZE; i++)
	{
		int r_flag = dEXT_CH__LLx__PUMPING_STS_FLAG_X[i]->When__DATA("END");
	...
	for(i=0; i<iDATA__LLx_SIZE; i++)
	{
		if(dEXT_CH__LLx__PUMPING_STS_FLAG_X[i]->Check__DATA("PUMPING") < 0)
			continue;
	...
	for(i=0; i<iDATA__LLx_SIZE; i++)
	{
		if(pLLx__OBJ_PHY_X[i]->Is__OBJ_BUSY() > 0)		continue;

		pLLx__OBJ_PHY_X[i]->Call__OBJECT("CL.PUMP_VLV");
	}
	...
	for(i=0; i<iDATA__LLx_SIZE; i++)
	{
		...
		if(sEXT_CH__LLx__PRESSURE_CTRL_FLAG_X[i]->Check__DATA("PUMP") < 0)
			continue;

		if(dEXT_CH__LLx__PRESSURE_STATUS_X[i]->Check__DATA(STR__VAC) < 0)
			continue;

		pLLx__OBJ_PHY_X[i]->Call__OBJECT("OP.PUMP_VLV");
	}
...
::Call__LEAK_CHECK(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	r_flag = Call__PUMP(p_variable,p_alarm);
...
::Fnc__LEAK_CHECK(CII_OBJECT__VARIABLE* p_variable, CII_OBJECT__ALARM* p_alarm)
{
	...
	// ...
	{
		...
		pTMC_CHM__OBJ_CTRL->Call__OBJECT("CL.PUMP_VLV");
	}
	...
	// ...
	{
		...
		aCH__CFG_TMC_LEAK_CHECK__ALARM_RATE->Get__DATA(var_data);
