...
LOOP_RETRY:
	...
	if(bActive__LLx_MULTI_SLOT_VALVE)
	{
		for(i=0; i<iCFG__LLx_SIZE; i++)
		{
			...
			{
				if(dEXT_CH__LLx__SLIT_VALVE_STATUS_XX[i][k]->Check__DATA(STR__CLOSE) < 0)
				{
					err_msg.Format("LL%1d-%1d", i+1,k+1);
	...
	else
	{
		for(i=0; i<iCFG__LLx_SIZE; i++)
		{
			if(dEXT_CH__LLx__SLIT_VALVE_STATUS_A[i]->Check__DATA(STR__CLOSE) < 0)
			{
				err_msg.Format("LL%1d", i+1);
				l_err_msg.Add(err_msg);
			}
		}
	}
