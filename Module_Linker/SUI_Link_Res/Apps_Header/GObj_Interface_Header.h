...
public:
	...
	//---------------------------------------------------------
	virtual int Set_Suffix_Feedback_Of_Channel(const CString& ch_name,const CString& ch_data) = 0;
	virtual int Get_Channel_Data_With_Check_Box_Of_Channel(const CString& ch_name,CString& ch_data) = 0;

	//---------------------------------------------------------
	virtual CString Get__Page_Name() = 0;

	virtual int Save__Config_File(const CStringArray& l_channel,
								  const CStringArray& l_data) = 0;
	virtual int Undo__Config_File(CStringArray& l_channel,
								  CStringArray& l_data) = 0;

	virtual int Get__Config_Backup_File(CStringArray& l_file,
										CStringArray& l_date) = 0;
	virtual int Get__Comare_Config_File(const CString& backup_file, 
										CStringArray& l_channel,
										CStringArray& l_data) = 0;

	//---------------------------------------------------------
	virtual int Callback__Window_Command(const CString& cmmd_name, 
										 const CStringArray& l_para,
										 CStringArray& l_rsp1,
										 CStringArray& l_rsp2,
										 CStringArray& l_rsp3) = 0;

	virtual int Set_Suffix_Feedback_With_Comment(const CString& key_word,
											     const CString& ch_data,
												 const CString& str_comment) = 0;

	//---------------------------------------------------------
	virtual int Undo__Config_Of_File_Path(const CString& file_path,
										  CStringArray& l_channel,
								          CStringArray& l_data) = 0;

	virtual int Undo__Config_Ex(const CString& page_name,
								const int eqp_id,
